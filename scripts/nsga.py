...
foldList = []
subjectList = []
for (foldFile,subjectFile) in zip(sorted(glob.glob(path+"kFolds/*Train.txt")), sorted(glob.glob(path+"underlyingNetworks/*.csv")*3)):
...
G = nx.from_numpy_matrix(np.genfromtxt(underlyingPath,delimiter=','))  # load squareform connectivity matrix as numpy matrix
...
def main():
    ...
    emostats = tools.Statistics()
    emostats.register("pop", copy.deepcopy)
...
if __name__ == '__main__':
    ...
    pop, logbook = main()
    pickle.dump(logbook, open(filePath[-19:-4] + '_logbook.pickle', 'wb'))
